...
const ctx = canvas.getContext('2d'); //fornece os métodos e prioridades necessárioas para desenhar e manipular gráficos 2D
...
function drawSnake() {
    //Define a cor de preenchimento para a cobrinha (cinza)
ctx.fillStyle = '#00ff08';
  ...
  snake.forEach(segment => {
    ...
    //As coordenadas do retangulo são baseadas nas coordenadas do segmento da cobrinha
...
function moveSnake() {
    if (!paused) { // Verifica se o jogo não está pausado
        ...
        if (head.x === food.x && head.y === food.y) { // Verifica se a cabeça da cobrinha comeu a comida
            generateFood(); // Gera a nova comida
        } else {
            snake.pop; // Remove a última parte da cauda da cobrinha
        }
        if (checkCollision()) { //Verifica a colisão da cobrinha consigo mesma ou com as bordas do canvas
...
function checkCollision() {
    ...
    for (let i = 1; i < snake.length; i++) {
        if (snake[i].x === head.x && snake[i].y === head.y) {
            return true; //Retorna true se houver colição com o própro corpo da cobrinha
        }
    }
...
pauseButton.addEventListener('click', () => {
    ...
    //se 'paused' for falso, o texto do botão será 'Paused', indicando que o jogo está em execução e pode ser pausado
